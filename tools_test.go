...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		if e.errorExpected && err == nil {
			t.Errorf("%s: error expected but non recevied", e.name)
		}

		if !e.errorExpected && err != nil {
			t.Errorf("%s: error not expected but one recevied: %s", e.name, err.Error())
		}
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if rr.Code != http.StatusServiceUnavailable {
		t.Errorf("wrong status cod returned; expected 503, but got %d", rr.Code)
	}
}
